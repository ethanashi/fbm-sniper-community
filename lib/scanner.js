...
import fs from "fs";
import { searchMarketplace, getListingDetail, mergeDetail, preloadMarketplaceMetadata } from "./fb-scraper.js";
...
import { formatTargetLine, inferTargetType, isSearchPlaceholderListing, matchesKeyword } from "./target-utils.js";
...
function loadConfig() {
  ...
  try {
    ...
    const merged = {
      ...
      location: normalizeLocation(parsed && parsed.location),
    };
...
function hasConfirmedLocation(loc) {
  return hasValidLocation(loc) && loc.confirmed === true;
}

function isLegacyMadridLocation(loc) {
  if (!hasValidLocation(loc)) return false;
  if (loc.confirmed === true) return false;
  const label = String(loc.label || "").trim().toLowerCase();
  return nearNumber(loc.latitude, LEGACY_MADRID_LOCATION.latitude)
    && nearNumber(loc.longitude, LEGACY_MADRID_LOCATION.longitude)
    && (!label || label.includes("madrid") || label.includes("spain"));
}

function isDallasStarterLocation(loc) {
  return hasValidLocation(loc)
    && nearNumber(loc.latitude, DALLAS_LOCATION.latitude)
    && nearNumber(loc.longitude, DALLAS_LOCATION.longitude);
}

function normalizeLocation(raw) {
  const loc = raw && typeof raw === "object" && !Array.isArray(raw) ? raw : {};
  if (!hasValidLocation(loc) || isLegacyMadridLocation(loc)) {
    return { ...DEFAULT_CONFIG.location };
  }

  const label = String(loc.label || "").trim();
  const confirmed = typeof loc.confirmed === "boolean"
    ? loc.confirmed
    : !isDallasStarterLocation(loc);

  return {
    label: label || `${Number(loc.latitude)}, ${Number(loc.longitude)}`,
...
function mergeSharedLocationConfig(config, sharedConfig) {
  const merged = { ...config };
  const sharedLoc = sharedConfig?.location || null;
  if (!hasConfirmedLocation(merged.location) && hasConfirmedLocation(sharedLoc)) {
    merged.location = {
      label: sharedLoc.label || "Shared marketplace location",
      latitude: Number(sharedLoc.latitude),
      longitude: Number(sharedLoc.longitude),
      confirmed: true,
    };
  }

  if (!merged.proxy && typeof sharedConfig?.proxy === "string" && sharedConfig.proxy.trim()) {
    merged.proxy = sharedConfig.proxy.trim();
  }
  if ((!Array.isArray(merged.proxyPool) || !merged.proxyPool.length) && Array.isArray(sharedConfig?.proxyPool)) {
    merged.proxyPool = sharedConfig.proxyPool.slice();
  }

  const sharedFbLocId = String(sharedConfig?.fbMarketplaceLocationId || "").trim();
  if (sharedFbLocId && !merged.fbMarketplaceLocationId) {
    merged.fbMarketplaceLocationId = sharedFbLocId;
  }
...
function loadEffectiveConfig() {
  ...
  try {
    return mergeSharedLocationConfig(config, loadWorkspaceConfig());
  } catch {
    return config;
  }
}
...
function formatProxyLabel(proxyUrl) {
  ...
  return normalizeProxyInput(proxyUrl)?.label || "proxy";
}
...
function matchesTargetRules(listing, watch) {
  ...
  if (mustInclude.length && !mustInclude.every((keyword) => matchesKeyword(text, keyword))) {
    return false;
  }

  if (mustAvoid.some((keyword) => matchesKeyword(text, keyword))) {
    return false;
  }
...
async function runOnce() {
  ...
  if (!hasConfirmedLocation(config.location)) {
    throw new Error("Location needs review. Open Settings, change or confirm your shared/cars city + coordinates, then start the bot again.");
  }
  ...
  console.log(chalk.dim(`  Active targets: ${activeTargets.length}/${watchlist.length}${capped ? ` (capped at ${MAX_ACTIVE_TARGETS})` : ""}`));
  ...
  console.log(chalk.dim(`  Photo grading: rules-only`));

  let marketplaceMetadataSession = null;
  if (proxyPool.length) {
    try {
      marketplaceMetadataSession = await preloadMarketplaceMetadata({
        lat: Number(config.location.latitude),
        lng: Number(config.location.longitude),
      });
      if (marketplaceMetadataSession) {
        console.log(chalk.dim("  Facebook metadata: direct bootstrap ready for proxied searches"));
      } else {
        console.warn(chalk.yellow("  Facebook metadata preload did not expose a search doc_id; falling back to per-proxy bootstrap."));
      }
    } catch (error) {
      console.warn(chalk.yellow(`  Facebook metadata preload failed: ${error.message}`));
    }
  }
  ...
  await runPool(activeTargets.map((watch, index) => ({ watch, index })), searchConcurrency, async ({ watch, index }) => {
    ...
    const { listings } = await searchMarketplace({
      ...
      conditions,
      proxyUrl,
      metadataSession: marketplaceMetadataSession,
      fallbackToDirectOnProxyRateLimit: true,
      fbLocationId: String(config.fbMarketplaceLocationId || "").trim() || null,
    });
    const bootstrapLocation = { latitude: lat, longitude: lng };
    const sessionCookies =
      getCachedSession(proxyUrl, bootstrapLocation)?.cookies ||
      getCachedSession(null, bootstrapLocation)?.cookies ||
      marketplaceMetadataSession?.cookies ||
      getCachedSession(proxyUrl)?.cookies;
