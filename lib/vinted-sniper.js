...
import { loadWorkspaceConfig, loadWorkspaceWatchlist, ensureWorkspaceFiles, resolveTargetForPlatform, hasValidLocation, vintedDomainInfo, VINTED_DOMAINS } from "./shared-marketplace/workspace.js";
...
async function runTargetQuery(target, cookie) {
...
async function runCycle(workspaceConfig) {
  ...
  for (let i = 0; i < targets.length; i++) {
    await runTargetQuery(resolveTargetForPlatform(targets[i], "vinted"), cookie);
...
async function main() {
  ...
  if (!hasValidLocation(loc)) {
    console.error(chalk.red("[vinted] No location set. Open Settings, enter your city + coordinates, then start the bot again."));
    process.exit(2);
  }
  ...
  console.log(chalk.blueBright(`  📍 ${loc.label || "?"} | 🌍 ${domainInfo.country} (${domainInfo.domain}) | ⏱  ${intervalSec}s interval`));
