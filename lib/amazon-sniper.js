...
async function scrapeAmazon(domain, query) {
  const cacheDir = process.env.PUPPETEER_CACHE_DIR || path.join(process.cwd(), 'data', 'puppeteer-cache');

  // Try to find the local chrome executable if it exists
  let executablePath = undefined;
  try {
    const { computeExecutablePath, Browser, detectBrowserPlatform } = await import('@puppeteer/browsers');
