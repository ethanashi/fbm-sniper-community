...
async function createMarketplacePage(proxyUrl = null) {
  let puppeteer;
  try {
...
function buildMarketplaceSearchUrl({ query, lat, lng, radiusKM } = {}) {
...
export async function scrapeMarketplaceSearchPage({
  ...
  const { browser, page } = await createMarketplacePage(proxyUrl);
  try {
    const url = buildMarketplaceSearchUrl({ query, lat, lng, radiusKM });
