...
import { convertPriceBandForCurrency, createCurrencyConverter, nativeCurrencyForPlatform, normalizeCurrencyCode } from "./shared-marketplace/currency.js";
...
function detectModel(text, product) {
  ...
  if (p === "playstation" || p === "console") return detectPSModel(text);
...
function isSpecializedFacebookProduct(product) {
  const p = String(product || "").toLowerCase();
  return p === "iphone" || p === "playstation" || p === "console";
}

function genericListingLabel(target, listing) {
  return String(target?.label || target?.query || listing?.title || "Marketplace item").trim() || "Marketplace item";
}
...
async function runQuery(target, cfg) {
  ...
  const searchCurrency = nativeCurrencyForPlatform("facebook", { facebookCurrency: cfg.displayCurrency });
  const nativePriceBand = convertPriceBandForCurrency(priceBand, cfg.money, searchCurrency);
  let result;
  try {
    result = await searchMarketplace({
      ...
      minPrice: nativePriceBand.minPrice * 100,
      maxPrice: nativePriceBand.maxPrice * 100,
      sort: "CREATION_TIME_DESCEND",
      maxPages: 1,
      conditions: ["new", "used_like_new", "used_good"],
      daysSinceListed: 1,
      fbLocationId: cfg.fbLocationId,
    });
  } catch (err) {
  ...
  for (const listing of listings) {
    ...
    const { price: nativePrice } = listing;
    const nativeCurrency = normalizeCurrencyCode(listing.currency, searchCurrency);
    if (!nativePrice) continue;
    const price = cfg.money.toDisplay(nativePrice, nativeCurrency);
    ...
    const isSpecialized = isSpecializedFacebookProduct(product);
    let modelKey = null;
    let storageGb = null;
    ...
    if (isSpecialized) {
      ...
      storageGb = product === "iphone" ? (detectStorage(text) ?? defaultStorage(modelKey)) : null;
      const nativeMaxBuy = resolveMaxBuy(product, modelKey, storageGb);
      maxBuy = nativeMaxBuy === null ? maxBuy : cfg.money.toDisplay(nativeMaxBuy, "EUR");
    }

    if (maxBuy !== null && price > maxBuy * LOWBALL_MAX_RATIO) continue;

    candidates.push({ listing, modelKey, storageGb, maxBuy, product, isSpecialized, price, nativePrice, nativeCurrency });
  }
  ...
  const bootstrapLocation = { latitude: cfg.lat, longitude: cfg.lng };
  const sessionCookies =
    getCachedSession(null, bootstrapLocation)?.cookies ||
    getCachedSession()?.cookies;
  const withDetails = await Promise.all(candidates.map(async (c) => {
    try {
      const raw = await getListingDetail(c.listing.id, sessionCookies, null, bootstrapLocation);
  ...
  // Phase 3 — score + log
  for (const { listing, modelKey, storageGb, maxBuy, product, isSpecialized, price, nativePrice, nativeCurrency } of withDetails) {
    ...
    const p = product;
    const modelLabel = isSpecialized
      ? (p === "playstation" || p === "console" ? modelKey.toUpperCase() : titleCase(modelKey))
      : genericListingLabel(target, listing);
    const sLabel = isSpecialized && p === "iphone" ? ` ${storageLabel(storageGb)}` : "";
    ...
    if (verdict.grade === "A" || verdict.grade === "B") {
      ...
      console.log(chalk.green(`📱 ${modelLabel}${sLabel} — ${cfg.money.format(price)}`));
      if (maxBuy && savings !== null) {
        console.log(chalk.green(`💰 Max buy: ${cfg.money.format(maxBuy)} | Save: ${cfg.money.format(savings)}`));
      }
      console.log(chalk.green(`🔗 ${listing.url}`));
      console.log(chalk.green(border));
      process.stdout.write("\x07");
    } else {
      const savings2 = savings !== null ? (savings >= 0 ? `save ${cfg.money.format(savings)}` : `offer ${cfg.money.format(maxBuy)}`) : "";
      console.log(chalk.yellow(`🎯 [fb/${verdict.grade}] ${modelLabel}${sLabel} — ${cfg.money.format(price)}${savings2 ? " · " + savings2 : ""}`));
    ...
    const record = {
      ...
      listing_price: price,
      currency: cfg.money.displayCurrency,
...
async function runCycle(cfg) {
  ...
  for (const target of targets) {
    await runQuery(resolveTargetForPlatform(target, "facebook"), cfg);
  }
...
async function main() {
  ...
  const cfg = {
    ...
    fbLocationId: String(workspaceConfig.fbMarketplaceLocationId || "").trim() || null,
  };
  cfg.money = await createCurrencyConverter({
    displayCurrency: workspaceConfig.displayCurrency,
