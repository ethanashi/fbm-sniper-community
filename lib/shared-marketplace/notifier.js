...
function collectPhotoUrls(record, maxPhotos) {
  ...
  return sources
    .map((photo) => {
      ...
      return photo?.full_size_url || photo?.url || photo?.imageUrl || "";
    })
    .filter(Boolean)
    .slice(0, maxPhotos);
}
...
export async function notify(record) {
  const notifications = resolveNotificationConfig(loadWorkspaceConfig());
  ...
  for (const target of targets) {
    try {
      await axios.post(target.webhookUrl, payload, { timeout: 15000 });
      routes.push(target.name);
    } catch (error) {
      console.error(`[notify] ${target.name} failed: ${error.message}`);
    }
  }

  if (routes.length) maybeOpenBrowser(record?.url, notifications);
