 ...
 *   3. Auto-fetched from vinted.es homepage (refreshed every 55 min)
 *
 * Usage:
...
import { loadWorkspaceConfig, loadWorkspaceWatchlist, ensureWorkspaceFiles } from "./shared-marketplace/workspace.js";
...
const VINTED_BASE   = "https://www.vinted.es";
...
async function runCycle(workspaceConfig) {
  ...
  // Reload watchlist each cycle so live edits take effect immediately
  ...
  for (let i = 0; i < targets.length; i++) {
    await runTargetQuery(targets[i], cookie);
...
async function main() {
  ...
  console.log(chalk.blueBright(`  📍 ${loc.label || "Spain"} | ⏱  ${intervalSec}s interval`));
