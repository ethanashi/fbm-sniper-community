...
function listedCeiling(maxAllIn, {
  ...
  verification = FEE_ELECTRONICS,
  protectionFlat = FEE_PROTECTION_FLAT,
  protectionPct = FEE_PROTECTION_PCT,
} = {}) {
  const net = maxAllIn - shipping - verification - protectionFlat;
  if (net <= 0) return 0;
  return net / (1 + protectionPct);
}

function computeFees(listed, shippingActual, {
  verificationFee = FEE_ELECTRONICS,
  shippingFallback = FEE_SHIPPING,
  protectionFlat = FEE_PROTECTION_FLAT,
} = {}) {
  const protection = protectionFlat + listed * FEE_PROTECTION_PCT;
  const shipping   = shippingActual != null ? shippingActual : shippingFallback;
  const round2 = (n) => Math.round(n * 100) / 100;
  return {
    protection: round2(protection),
    shipping: round2(shipping),
    total: round2(listed + protection + shipping),
    totalWithVerif: round2(listed + protection + shipping + verificationFee),
  };
}
...
function isSpecializedVintedProduct(product) {
  return String(product || "").toLowerCase() === "iphone";
}

function genericListingLabel(target, item) {
  return String(target?.label || target?.query || item?.title || "Vinted item").trim() || "Vinted item";
}
...
function parseProxyUrl(raw) {
  ...
  const cfg = { protocol: proxy.protocol, host: proxy.host, port: proxy.port };
  if (proxy.username) cfg.auth = { username: proxy.username, password: proxy.password || "" };
  return cfg;
}
...
function extractToken(cookieStr) {
  ...
  // If the cookie string contains the key, extract the value; otherwise
  // assume the user pasted just the raw token and use the whole string.
  return m ? m[1] : s;
}
...
function loadManualCookie(workspaceConfig) {
  ...
  // If the user pasted only the bare token (no access_token_web= prefix),
  // reconstruct a well-formed cookie value so the Cookie header is correct.
...
async function searchListings(cookie, { keyword, priceFrom = 0, priceTo = 800, generic = false, currency = "EUR" }) {
  const params = {
    search_text: keyword,
  ...
  let response = await axios.get(CATALOG_URL(), {
  ...
  if (response.status === 403) {
    ...
    // caller will get fresh cookie next cycle
    return [];
  }
...
async function getItemDetail(itemId, cookie) {
  try {
    let response = await axios.get(ITEM_URL(itemId), {
      headers: vintedHeaders(cookie), proxy: nextProxy(), timeout: 20000, validateStatus: () => true,
    });
    if (response.status === 403) {
      await new Promise((r) => setTimeout(r, 10000));
      response = await axios.get(ITEM_URL(itemId), {
        headers: vintedHeaders(cookie), proxy: nextProxy(), timeout: 20000, validateStatus: () => true,
      });
    }
...
async function runTargetQuery(target, cookie, money, nativeCurrency = "EUR") {
  ...
  const isSpecialized = isSpecializedVintedProduct(product);
  const items = await searchListings(cookie, {
    keyword,
    priceFrom: nativePriceBand.minPrice,
    priceTo: nativePriceBand.maxPrice,
    generic: !isSpecialized,
  ...
  for (const item of items) {
    ...
    const listedNative = extractPrice(item);
    if (!Number.isFinite(listedNative) || listedNative <= 0) continue;
    if (listedNative < nativePriceBand.minPrice || listedNative > nativePriceBand.maxPrice) continue;
    const listed = money.toDisplay(listedNative, nativeCurrency);
    if (!Number.isFinite(listed) || listed < priceBand.minPrice || listed > priceBand.maxPrice) continue;

    if (!targetMatchesText(target, `${item.title || ""} ${item.description || ""}`)) continue;

    let modelKey = null;
    let storageGb = null;
    let maxAllIn = resolveTargetReferencePrice(target);
    let ceiling = maxAllIn;
    let modelLabel = genericListingLabel(target, item);

    if (isSpecialized) {
      modelKey = detectModel(item.title || "");
      if (!modelKey) continue;

      storageGb = detectStorageGb(`${item.title || ""} ${item.description || ""}`);
      const nativeMaxAllIn = resolveMaxBuy(modelKey, storageGb);
      maxAllIn = nativeMaxAllIn === null ? maxAllIn : money.toDisplay(nativeMaxAllIn, "EUR");
      ceiling = maxAllIn === null ? null : listedCeiling(maxAllIn, {
        shipping: money.toDisplay(FEE_SHIPPING, "EUR"),
        verification: money.toDisplay(FEE_ELECTRONICS, "EUR"),
        protectionFlat: money.toDisplay(FEE_PROTECTION_FLAT, "EUR"),
      });
      modelLabel = modelKey.split(" ").map((w) =>
        w === "iphone" ? "iPhone" : w.charAt(0).toUpperCase() + w.slice(1)
      ).join(" ");
    }
    if (ceiling !== null && listed > ceiling * LOWBALL_MAX_RATIO) continue;

    candidates.push({ item, listed, listedNative, modelKey, storageGb, maxAllIn, ceiling, modelLabel, isSpecialized, product });
  }
  ...
  for (let i = 0; i < candidates.length; i++) {
    const { item, listed, listedNative, storageGb, maxAllIn, ceiling, modelLabel, isSpecialized, product } = candidates[i];
    const detail = details[i];
    const shippingNative = extractShippingPrice(detail);
    const shippingDisplay = shippingNative == null ? null : money.toDisplay(shippingNative, nativeCurrency);
    ...
    const fees = computeFees(listed, shippingDisplay, {
      shippingFallback: money.toDisplay(FEE_SHIPPING, "EUR"),
      verificationFee: isSpecialized ? money.toDisplay(FEE_ELECTRONICS, "EUR") : 0,
      protectionFlat: money.toDisplay(FEE_PROTECTION_FLAT, "EUR"),
    });
    ...
    const savings = ceiling !== null ? Math.round(ceiling - listed) : null;
    ...
    if (verdict.grade === "A" || verdict.grade === "B") {
      ...
      console.log(chalk.green(`📱 ${modelLabel}${storageLbl ? " " + storageLbl : ""} — ${money.format(listed)}`));
      console.log(chalk.green(`💰 All-in: ${money.format(fees.total)} | w/verif: ${money.format(fees.totalWithVerif)}${maxAllIn !== null ? ` | Max all-in: ${money.format(maxAllIn)}` : ""}`));
      if (savings !== null && savings > 0) console.log(chalk.green(`💸 Under ceiling by ${money.format(savings)}`));
      ...
      console.log(chalk.yellow(`🎯 [vt/${verdict.grade}] ${modelLabel}${storageLbl ? " " + storageLbl : ""} — ${money.format(listed)} · all-in ${money.format(fees.totalWithVerif)}`));
    ...
    const record = {
      ...
      native_listing_price: listedNative,
...
async function runCycle(workspaceConfig, money, nativeCurrency) {
  cycleCount++;
  ...
  for (let i = 0; i < targets.length; i++) {
    await runTargetQuery(resolveTargetForPlatform(targets[i], "vinted"), cookie, money, nativeCurrency);
...
async function main() {
  ...
  const nativeCurrency = currencyForVintedDomain(domainInfo.domain);
  const money = await createCurrencyConverter({
    displayCurrency: workspaceConfig.displayCurrency,
    sourceCurrencies: ["EUR", nativeCurrency],
  });
  ...
  await runCycle(workspaceConfig, money, nativeCurrency);
  if (FLAG_TEST) { console.log(chalk.green("[vinted] Test complete.")); process.exit(0); }

  while (running) {
    await new Promise((r) => setTimeout(r, intervalSec * 1000));
    if (running) await runCycle(workspaceConfig, money, nativeCurrency);
  }
}
