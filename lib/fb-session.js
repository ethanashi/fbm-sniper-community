...
export async function bootstrapSession(proxyUrl = null) {
  ...
  cdp.on('Network.requestWillBeSent', (event) => {
    ...
    if (docMatch) {
      ...
      if (/CometMarketplaceSearchContentContainerQuery/i.test(name)) {
        discoveredDocIds.searchContent = docId;
      }
      if (/CometMarketplaceSearchRootQuery/i.test(name)) {
      ...
      // Capture the actual search variables FB used
      const varsMatch = postData.match(/variables=([^&]+)/);
      if (varsMatch && /marketplace.*search/i.test(name)) {
        try {
          discoveredDocIds._capturedVariables = JSON.parse(decodeURIComponent(varsMatch[1]));
        } catch (_) { /* skip */ }
      }
      // Keep a "search" alias pointing to the best candidate
    ...
    await page.goto(
      'https://www.facebook.com/marketplace/madrid/search/?query=iphone&sortBy=creation_time_descend',
    ...
    if (!discoveredDocIds.search) {
      console.warn(
        ...
        '  This likely means Facebook changed their code structure.\n' +
        '  Try running with a non-headless browser to manually inspect.'
      );
    }
