...
async function runQuery(target, cfg) {
  ...
  try {
    items = await fetchPage(query, {
      ...
      minPrice: Number.isFinite(Number(target?.minPrice)) ? Number(target.minPrice) : 80,
      maxPrice: Number.isFinite(Number(target?.maxPrice)) ? Number(target.maxPrice) : 3500,
    });
    backoffSec = 0;
  } catch (err) {
