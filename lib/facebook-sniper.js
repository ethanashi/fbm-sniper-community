...
import { loadWorkspaceConfig, loadWorkspaceWatchlist, ensureWorkspaceFiles, resolveTargetForPlatform, hasValidLocation } from "./shared-marketplace/workspace.js";
...
async function main() {
  ...
  if (!hasValidLocation(loc)) {
    console.error(chalk.red("[facebook] No location set. Open Settings, enter your city + coordinates, then start the bot again."));
    process.exit(2);
  }
  ...
  console.log(chalk.blueBright(`  📍 ${loc.label || `${cfg.lat},${cfg.lng}`} | ⏱  ${cfg.intervalSec}s interval`));
